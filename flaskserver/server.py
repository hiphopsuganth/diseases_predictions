...
# Helper function to retrieve additional info for a disease
def helper(dis):
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:postgres@localhost/herbal'
...
# Endpoint to submit patient details
...
def predict():
    ...
    # Use helper function to get details for the predicted disease
    precautions, medications, diets, workouts = helper(prediction)
    
    # Construct the response as a JSON object
    response = {
        'disease': prediction,
        'precautions': [str(p) for p in precautions],
...
# Endpoint to fetch description and image from Wikipedia
...
def fetch_info():
    ...
    description = ' '.join([para.text for para in paragraphs[:2]])  # Fetch first two paragraphs
    clean_description = re.sub(r'\[\d+\]', '', description)  # Remove reference numbers
    
    # Extract images
    images = soup.find_all('img')
    image_urls = ["https:" + img['src'] for img in images if img['src'].startswith('//upload.wikimedia.org')]
    
    if image_urls:
        response = jsonify({'description': clean_description, 'urls': image_urls[:5]})
        response.headers.add("Access-Control-Allow-Origin", "*")
        return response
    else:
        response = jsonify({'error': 'No images found'})
        response.headers.add("Access-Control-Allow-Origin", "*")
        return response, 404
